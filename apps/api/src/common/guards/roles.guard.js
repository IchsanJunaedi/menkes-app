    ...
    var RolesGuard = class {
        ...
        canActivate(context) {
            ...
            if (!user || (!user.roles && !user.realm_access?.roles)) {
                throw new common_1.ForbiddenException('User does not have any assigned roles');
            }
            ...
            if (!hasRole) {
                throw new common_1.ForbiddenException(`Access forbidden. Required roles: ${requiredRoles.join(', ')}`);
            }
            return true;
        }
    };
